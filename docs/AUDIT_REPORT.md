...
| Seeded reproducibility | working with caveats | `src/lib/utils/random.js`, `src/lib/geo/generators.js`, `src/app/generate/components/GenerateMap.js`, `src/app/generate/page.js` | Module-level mulberry32 RNG seeded at the top of every generation run. Seed input always visible in the sidebar; word seeds are hashed. Temporal attributes previously bypassed the seeded RNG via `Math.random()` (post-audit finding); now routed through `getRandomFloat` so datasets with a temporal field are reproducible. Still not covered by an automated regression test. | high |
...
| Build / static export | working | `npm run build` green as of commit `4d1e012` (re-run after the temporal-RNG fix and `appVersion` source change) | Build completes successfully and exports `/`, `/about`, and `/generate` | high |
